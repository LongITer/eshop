        ...
        <div className="w-full flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-[270px] !rounded bg-white p-4 space-y-6 shadow-md">
              ...
              <Range
                ...
                renderTrack={({ props, children }) => (
                  <div {...props} className="h-[6px] bg-blue-200 rounded relative" style={{ ...props.style }}>
                    <div className="absolute h-full bg-blue-600 rounded" style={{ left: `${((tempPriceRange[0] - MIN) / (MAX - MIN)) * 100}%`, right: `${100 - ((tempPriceRange[1] - MIN) / (MAX - MIN)) * 100}%` }} />
                    {children}
                  </div>
                )}
                renderThumb={({ props }) => (
                  <div {...props} className="h-5 w-5 bg-blue-600 rounded-full shadow-md focus:outline-none" style={{ ...props.style }} />
                )}
              />
              <div className="flex justify-between mt-3">
                <div className="text-sm text-gray-600">${tempPriceRange[0]} - ${tempPriceRange[1]}</div>
                <button onClick={() => { setPriceRange(tempPriceRange); setPage(1); }} className="text-sm px-4 py-1 bg-gray-200 hover:bg-blue-600 hover:text-white transition !rounded">
                  Apply Filter
                </button>
              </div>
            </div>
            ...
            <ul className="space-y-2 !mt-3">
              {isLoading ? <p>Loading ...</p> : data?.categories?.map((category: any) => (
            ...
            <h3 className="text-xl font-Poppins font-medium border-b border-b-slate-300 pb-1">Color</h3>
            <ul className="space-y-2 !mt-3">
              {colors.map((color) => (
            ...
            <h3 className="text-xl font-Poppins font-medium border-b border-b-slate-300 pb-1">Sizes</h3>
            <ul className="space-y-2 !mt-3">
              {sizes.map((size) => (
          ...
          <div className="flex-1 min-w-0 px-2 lg:px-3">
            {isProductLoading ? (
              <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
                {Array.from({ length: 10 }).map((_, index) => <div key={index} className="h-[250px] bg-gray-300 animate-pulse rounded-xl"></div>)}
              </div>
            ) : products.length > 0 ? (
              <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
            ...
            {totalPages > 1 && (
              <div className="flex justify-center mt-8 gap-2">
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button key={i + 1} onClick={() => setPage(i + 1)} className={`px-3 py-1 !rounded border border-gray-200 text-sm ${page === i + 1 ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700"}`}>
                    {i + 1}
                  </button>
                ))}
              </div>
