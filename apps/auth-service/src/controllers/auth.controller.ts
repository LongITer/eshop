...
export const refreshUserToken = async (
  ...
  try {
    ...
    if (!decoded || !decoded.id || decoded.role !== "user") {
      return next(
        new JsonWebTokenError("Forbidden! Invalid user refresh token!"),
      );
    }
...
export const refreshSellerToken = async (
  ...
  try {
    ...
    if (!token) {
      return next(
        new ValidationError("Unauthorized! No seller refresh token!"),
      );
    }
    ...
    if (!decoded || !decoded.id || decoded.role !== "seller") {
      return next(
        new JsonWebTokenError("Forbidden! Invalid seller refresh token!"),
      );
    }
    ...
    if (!stripeAccountId) {
      ...
      try {
        ...
        return next(
          new AppError(
            stripeError.message ||
              "Failed to create Stripe account. Please ensure Connect is enabled in your Stripe Dashboard.",
            400,
          ),
        );
      }
